...
class _ViewCityState extends State<ViewCity> {
  submitData(){
    ...
    databaseReference.child("country").child(widget.countryId).child("city").push().set({
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (BuildContext context) => ViewCity(widget.countryId)));
  ...
  Future<List<PropertyType>> getSlideList() async {
    ...
    final databaseReference = FirebaseDatabase.instance.reference();
    await databaseReference.child("country").child(widget.countryId).child("city").once().then((DataSnapshot dataSnapshot){
              ...
              children: [
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(hintText:"Enter City",contentPadding: EdgeInsets.only(left: 10)),
                  ),
                ),
                Container(
                  margin: EdgeInsets.all(10),
                  child: TextField(
                    ...
                    decoration: InputDecoration(hintText:"Enter City (Arabic)",contentPadding: EdgeInsets.only(left: 10)),
                  ),
                ),
                Container(
                    margin: EdgeInsets.all(10),
                ...
                child: Stack(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text("City",style: TextStyle(fontWeight: FontWeight.w700,fontSize: 13),),
                    ),
                                        ...
                                        children: [
                                          Container(
                                              ...
                                              child: RaisedButton(
                                                onPressed: ()async{
                                                  final databaseReference = FirebaseDatabase.instance.reference();
                                                  await databaseReference.child("country").child(widget.countryId).child("city").child(snapshot.data[index].id).remove().then((value) {
                                                    Navigator.pushReplacement(
                                                        context, MaterialPageRoute(builder: (BuildContext context) => ViewCity(widget.countryId)));
                                                  });
                                                },
                                                color: Colors.red,
                                          ...
                                          Container(
                                              ...
                                              child: RaisedButton(
                                                onPressed: (){
                                                  Navigator.pushReplacement(
                                                      context, MaterialPageRoute(builder: (BuildContext context) => ViewArea(widget.countryId,snapshot.data[index].id)));
                                                },
                                                color: primaryColor,
                                                child: Text("View Areas",style: TextStyle(color: Colors.white),),
